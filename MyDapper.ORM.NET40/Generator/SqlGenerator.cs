    ...
    public class SqlGenerator : ISqlGenerator
    {
       ...
       public virtual char ParameterPrefix
        ...
        public virtual string GetInsertSql<T>()
        {
            ...
            return string.Format("INSERT INTO {0} ({1}) values ({2})", map.TableName, columnNames, parameters);
        }
        ...
        public virtual string GetSelectSql<T, W>(W where)
        {
            ...
            return string.Format("SELECT * FROM {0} WHERE {1} and {2}", mapT.TableName, EmptyExpression, strWhere);
        }
        ...
        {
            ClassMapper mapT = GetMapper(typeof(T));
            string set = mapT.Properties.Where(p => p.Name.ToLower() != mapT.PrimaryKey.ToLower())
        ...
        public virtual string GetDeleteSql<T, W>(W where)
        {
            ...
            ClassMapper mapW = GetMapper(where.GetType());
            string strWhere = mapW.Properties.Select(p => string.Format("{0}={1}{0}", p.Name, ParameterPrefix)).AppendStrings(" and ");
            return string.Format("DELETE FROM {0} WHERE {1} and {2}", mapT.TableName, EmptyExpression, strWhere);
        }
        ...
        public virtual string GetCountSql<T, W>(W where)
        {
            ...
            ClassMapper mapW = GetMapper(where.GetType());
            string strWhere = mapW.Properties.Select(p => string.Format("{0}={1}{0}", p.Name, ParameterPrefix)).AppendStrings(" and ");
            return string.Format("SELECT COUNT(1) FROM {0} WHERE {1} and {2}", mapT.TableName, EmptyExpression, strWhere);
        }
